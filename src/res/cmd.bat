...
set path=%cd%;%cd%\tool\Win;%path%;%windir%\Sysnative
set framework_workspace=%cd%

::ECHO.检查ECHOC...
if not exist %framework_workspace%\tool\Win\ECHOC.exe ECHO.找不到ECHOC.exe& goto FATAL
ECHOC | find "Usage" 1>nul 2>nul || ECHO.ECHOC.exe无法运行&& goto FATAL
::ECHO.检查strtofile...
if not exist %framework_workspace%\tool\Win\strtofile.exe ECHOC {%c_e%}找不到strtofile.exe{%c_i%}{\n}& goto FATAL
if exist %framework_workspace%\tmp\bff-test.txt del %framework_workspace%\tmp\bff-test.txt 1>nul
echo.bff-test|strtofile.exe %framework_workspace%\tmp\bff-test.txt || ECHOC {%c_e%}strtofile.exe无法运行{%c_i%}{\n}&& goto FATAL
for /f %%a in (%framework_workspace%\tmp\bff-test.txt) do (if not "%%a"=="bff-test" ECHOC {%c_e%}strtofile.exe无法运行{%c_i%}{\n}& goto FATAL)
...
:CMD-CONTINUE
ECHOC {0E}=--------------------------------------------------------------------={0F}{\n}
ECHOC {0E}[命令]{0F} & call strtofile.exe %framework_workspace%\tmp\cmd.bat
...
if "%cmd%"=="cfg" (
    ...
    goto CMD-CONTINUE)
call %framework_workspace%\tmp\cmd.bat || ECHOC {0C}                                                          [好像出错了]{0F}{\n}
goto CMD-CONTINUE


:pushlinuxtool
::if not exist %framework_workspace%\tool\Android\%1 ECHOC {0C}找不到%framework_workspace%\tool\Android\%1{0F}{\n}& goto :eof
adb.exe push %framework_workspace%\tool\Android\%1 ./%1 1>nul || ECHOC {0C}推送%framework_workspace%\tool\Android\%1到./%1失败{0F}{\n}&& goto :eof
adb.exe shell chmod 777 ./%1 1>nul || ECHOC {0C}授权./%1失败{0F}{\n}&& goto :eof
goto :eof
